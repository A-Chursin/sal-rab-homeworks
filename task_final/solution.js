...
function sendRequest(name, phone, address, goods, sum) {
    ...
    let goodsContainer = {title: '', count: 0} //создаем объект-контейнер

    for (let i = 0; i < countOfGoods; i += 1) {
        goodsContainer.title = goods[i].title;
        goodsContainer.count = goods[i].count;
        data.goods.push(goodsContainer);
        goodsContainer = {title: '', count: 0}; //очищаем контейнер, поскольку очевидно перезапись работает не так, как я представлял. Должен быть более элегантный способ...
    }
